...
def fetch_published_date_from_url(url: str) -> str | None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=4.0) as resp:
            ...
            # 1. ArXiv submission date: [Submitted on 17 Sep 2026]
            if "arxiv.org" in url:
                dateline_match = re.search(r"\[Submitted on\s+(\d{1,2}\s+[A-Za-z]+\s+\d{4})", html_chunk)
                if dateline_match:
                    try:
                        dt = datetime.strptime(dateline_match.group(1), "%d %b %Y")
            ...
            # 4. MediaPost & byline text fallback
    ...
    try:
        ...
        for card in cards:
            ...
            for item in card.find_all("div", class_="story-item"):
                ...
                # 2. Check story-meta (for Track A or inline dates like "Thu, 17 Sep")
                if delta_days is None and meta:
                    meta_text = meta.get_text()
                    date_candidate_match = re.search(
                        r"(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)[a-z]*,?\s+\d{1,2}\s+[A-Za-z]+",
                        meta_text,
                        re.IGNORECASE,
                    )
                    if date_candidate_match:
                        date_str_display = date_candidate_match.group(0).strip()
                        delta_days = parse_relative_or_abs_date(date_str_display, run_dt)

                # 3. Check Techmeme permalink slug (techmeme.com/260909/p29 -> 2026-09-09)
                ...
                # 4. Dynamic HTTP lookup (resolves real ArXiv [Submitted on ...] date or meta tags)
                ...
                # 5. Fallback for Runs 1-5 without tags
                ...
                # 6. Final fallback if blocked or unreachable
